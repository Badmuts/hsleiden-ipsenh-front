...
class Room extends Component {
  ...
  componentDidMount() {
    room(this.props.match.params.buildingId, this.props.match.params.roomId)
      ...
      .then(room =>
        this.setState({ chartData: this.processLogs(this.state.startDate) })
      )
    ...
    _.forEach(room.logs, function(log) {
      if (Moment(log.time).format("D") === startDate.format("D")) {
        ...
        if (data.datasets[0].data[Moment(log.time).format("H")] > 0) {
          var tempValueFromDataArray =
            data.datasets[0].data[Moment(log.time).format("H")];
          var average = (log.occupation + tempValueFromDataArray) / 2;
          data.datasets[0].data[Moment(log.time).format("H")] = _.floor(
            average
          );
        } else {
            ...
            ? <div className="pt-card pt-elevation-0 pt-interactive">
                <div className="row">
                  <div className="col-xs">
                    <h2>
                      <span
                        className="pt-icon-large pt-icon-timeline-bar-chart"
                        style={{ color: Colors.GRAY4 }}
                      />{" "}
                      Occupation vs. Expected occupation
                    </h2>
